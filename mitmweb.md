...
Args that start with '--' (eg. --wport) can also be set in a config file
(~/.mitmproxy/common.conf or ~/.mitmproxy/mitmweb.conf or specified via
...
Mitmweb:
  --wport PORT          Mitmweb port.
  --wiface IFACE        Mitmweb interface.
  --wdebug              Turn on mitmweb debugging
  --wsingleuser USER    Allows access to a a single user, specified in the
                        form username:password.
  --whtpasswd PATH      Allow access to users specified in an Apache htpasswd
                        file.
